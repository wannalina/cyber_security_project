    ...
    def __init__(self, parent, controller):
        ...
        def message():
            ...
            try:
                ...
                cur.execute(sql)
                hashed_password_b64 = cur.fetchone()[1]
                ...
                connection.rollback()
                return
            user_unique_password = (user_name+password_entry.get()).encode('utf-8')
            print("login user unique password:",user_unique_password)
            print("login user hash with encoded 64:",hashed_password_b64)
            # Decode hashed password from base64
            hashed_password_bytes = base64.b64decode(hashed_password_b64)
            print("login user hash:",hashed_password_b64)

            # Check if entered password matches hashed password
            if bcrypt.checkpw(user_unique_password, hashed_password_bytes):
...
class signButton(tk.Frame):
    def __init__(self, parent, controller):
        ...
        def message():
            if not unique_username(id_entry.get()):
                return
            elif len(password_entry.get()) < 10:
                showerror(
                    'Notification!', 'The password you entered is too short!\n Please enter a new pasword')
                return
            elif not password_policies(id_entry.get(), password_entry.get()):
                showerror(
                ...
                user_unique_password = (user_id+user_password).encode('utf-8')
                print("sign in user unique password:",user_unique_password)
                hashed_password_bytes = bcrypt.hashpw(user_unique_password, bcrypt.gensalt())
                print("sign in hash:",hashed_password_bytes)

                # Encode hashed password as base64
                hashed_password_b64 = base64.b64encode(hashed_password_bytes)
                print("sign in hash with encoded base64:",hashed_password_b64)

                try:
                    cur.execute("INSERT INTO users VALUES ('"+id_entry.get()+"','"+hashed_password_b64.decode()+"');")
                    connection.commit()
